...
로그인 토큰은 서버 프로세스 메모리가 아니라 JSON 파일의 `sessions` 테이블에 저장합니다. 따라서 Gunicorn 작업 프로세스가 여러 개여도 동일한 세션을 조회할 수 있습니다.
...
- 삭제된 측정 기록은 JSON 파일에서 영구 삭제되며 복구 기능은 포함하지 않습니다.


## BMI·혈압·공복 혈당 자동 계산

측정 기록 생성 시 Flask 서버가 입력값을 먼저 계산하고 분류한 뒤 JSON 파일에 저장합니다.
...
기존 JSON 파일 DB에 저장된 측정 기록도 서버 시작 시 새 분류 컬럼을 자동 추가하고 재계산합니다.
...
평균은 별도 누적 컬럼에 저장하지 않고 현재 JSON 파일 기록을 기준으로 서버가 계산합니다. 측정 기록 등록(POST), 전체 수정(PUT), 삭제(DELETE) 응답에도 갱신된 평균이 포함되며, 화면은 해당 작업 직후 기록과 평균을 다시 조회합니다.
...
이 프로젝트는 SQLite 대신 `data.json` 파일에 사용자와 측정 기록을 저장합니다.
...
실제 데이터 파일은 다음 경로를 사용합니다.

```text
로컬 실행: back/data.json
Docker:    /data/data.json
```

`data.json`이 없으면 서버 시작 시 자동으로 생성됩니다. 저장할 때는 임시 파일을 먼저 완성한 뒤 `data.json`으로 교체하므로, 쓰기 도중 원본 파일이 부분적으로 잘리는 위험을 줄였습니다.

### 기존 SQLite 데이터 자동 변환

Docker 볼륨에 기존 `/data/health_measurement.db`가 있고 `data.json`이 아직 없으면, 최초 실행 시 사용자와 측정 기록을 JSON으로 한 번 변환합니다. 변환 이후 모든 CRUD·검색·평균 계산은 `data.json`만 사용합니다.

### 세션

로그인 세션은 영구 데이터가 아니므로 서버 메모리에 저장합니다. 서버나 컨테이너가 재시작되면 다시 로그인해야 하지만 사용자와 측정 기록은 `data.json`에 남습니다.

### Gunicorn 프로세스 수

JSON 파일은 여러 프로세스가 동시에 수정하기에 적합하지 않으므로 Gunicorn은 워커 1개와 스레드 4개로 실행합니다.

```text
--workers 1 --threads 4
```

한 프로세스 안에서는 잠금으로 등록·수정·삭제를 순서대로 처리합니다.

### Git 제외

실제 사용자 데이터는 저장소에 커밋하지 않습니다.

```gitignore
data.json
back/data.json
health_measurement.db
```

대신 빈 구조를 보여주는 `data.example.json`만 커밋합니다.

### 데이터 초기화

Docker 볼륨까지 삭제하려면 다음 명령을 사용합니다.

```bash
docker compose down -v
```

이 명령은 `data.json`과 기존 SQLite 파일을 모두 삭제하므로 필요한 데이터가 있다면 먼저 백업해야 합니다.


## 키·몸무게 상한 검증

측정 등록 폼과 PUT 수정 폼에서 다음 값은 비정상 입력으로 처리합니다.
...
사용자가 해당 값을 입력한 뒤 다른 입력칸으로 이동하면 즉시 경고가 표시되고, 잘못된 입력칸으로 포커스가 돌아갑니다. 입력값은 선택된 상태가 되어 바로 다시 작성할 수 있습니다.

프론트엔드 검증을 우회해 직접 API를 호출하더라도 백엔드에서 동일한 기준으로 `400 validation` 응답을 반환합니다.
